...
def cpadashboardfilters():
    ...
    for revenuelist in revenuelists:
        if revenuelist.text==values["revenuefield"]:
            revenuelist.click()
    ...
    for industrylist in industrylists:
        if industrylist.text==values["industryfield"]:
            industrylist.click()
    ...
    for statelist in statelists:
        if statelist.text==values["statefield"]:
            statelist.click()
    ...
    for statuslist in statuslists:
        if statuslist.text==values["statusfield"]:
            statuslist.click()
    ...
    try:
        ...
        for row in range(1, rowcounts + 1):
            ...
            if (values["name"] and values["revenuefield"]) and (values["industryfield"] and values["statefield"]) in rowContent:
